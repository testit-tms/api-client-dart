...
class TestResultV2GetModel {
  /// Returns a new [TestResultV2GetModel] instance.
  ...
  AutoTestModelV2GetModel? autoTest;
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TestResultV2GetModel &&
    other.id == id &&
  ...
  @override
  String toString() => 'TestResultV2GetModel[id=$id, configurationId=$configurationId, workItemVersionId=$workItemVersionId, testRunId=$testRunId, outcome=$outcome, configuration=$configuration, autoTest=$autoTest, autoTestId=$autoTestId, message=$message, traces=$traces, startedOn=$startedOn, completedOn=$completedOn, runByUserId=$runByUserId, stoppedByUserId=$stoppedByUserId, testPointId=$testPointId, testPoint=$testPoint, comment=$comment, links=$links, attachments=$attachments, parameters=$parameters, properties=$properties]';
  ...
  /// Returns a new [TestResultV2GetModel] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TestResultV2GetModel? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TestResultV2GetModel[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TestResultV2GetModel[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return TestResultV2GetModel(
        ...
        configuration: ConfigurationModel.fromJson(json[r'configuration']),
        autoTest: AutoTestModelV2GetModel.fromJson(json[r'autoTest']),
  ...
  static List<TestResultV2GetModel> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TestResultV2GetModel>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TestResultV2GetModel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TestResultV2GetModel> mapFromJson(dynamic json) {
    final map = <String, TestResultV2GetModel>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TestResultV2GetModel.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TestResultV2GetModel-objects as value to a dart map
  static Map<String, List<TestResultV2GetModel>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TestResultV2GetModel>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TestResultV2GetModel.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
