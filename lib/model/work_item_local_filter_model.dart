...
class WorkItemLocalFilterModel {
  /// Returns a new [WorkItemLocalFilterModel] instance.
  ...
  /// Returns a new [WorkItemLocalFilterModel] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static WorkItemLocalFilterModel? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "WorkItemLocalFilterModel[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "WorkItemLocalFilterModel[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return WorkItemLocalFilterModel(
  ...
  static List<WorkItemLocalFilterModel> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <WorkItemLocalFilterModel>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = WorkItemLocalFilterModel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, WorkItemLocalFilterModel> mapFromJson(dynamic json) {
    final map = <String, WorkItemLocalFilterModel>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = WorkItemLocalFilterModel.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of WorkItemLocalFilterModel-objects as value to a dart map
  static Map<String, List<WorkItemLocalFilterModel>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<WorkItemLocalFilterModel>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = WorkItemLocalFilterModel.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
